...
export function registerListCommand(program: Command): void {
  program
    ...
    .action(async (options: { tag?: string; category?: string; confidence?: string; type?: string }) => {
      try {
...
async function listTerms(
  vaultPath: string,
  options: { tag?: string; category?: string; confidence?: string },
): Promise<void> {
...
async function listContent(
  ...
  for (const file of mdFiles) {
    ...
    const { data } = matter(content);
    const name = (data.title as string) || path.basename(file, '.md');
    const tags: string[] = Array.isArray(data.tags) ? (data.tags as string[]) : [];
  ...
  const pad = (s: string, len: number) => s.slice(0, len).padEnd(len);
  const sep = chalk.dim(
    `${'─'.repeat(w.name)}  ${'─'.repeat(w.tags)}  ${'─'.repeat(w.summary)}`,
  );

  console.log();
  console.log(
    `${chalk.bold(pad('Başlık', w.name))}  ${chalk.bold(pad('Tags', w.tags))}  ${chalk.bold(pad('Özet', w.summary))}`,
  );
  console.log(sep);

  for (const row of rows) {
    console.log(
      `${chalk.cyan(pad(row.name, w.name))}  ${chalk.dim(pad(row.tags, w.tags))}  ${chalk.white(pad(row.summary, w.summary))}`,
    );
  }
