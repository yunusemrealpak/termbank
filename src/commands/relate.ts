...
export function registerRelateCommand(program: Command): void {
  program
    ...
    .action(async (term1: string, term2: string) => {
      try {
        ...
        if (!added1) console.log(chalk.dim(`  ${name1} zaten ${name2}'yi bağlıyordu`));
        if (!added2) console.log(chalk.dim(`  ${name2} zaten ${name1}'yi bağlıyordu`));
      } catch (err) {
...
async function getDisplayName(vaultPath: string, searchTerm: string): Promise<string> {
  const content = await readTerm(vaultPath, searchTerm);
  if (!content) return searchTerm;
  const { data } = matter(content);
  return (data.term as string) || searchTerm;
}
