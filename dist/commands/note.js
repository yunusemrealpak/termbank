...
import { getVaultContext, noteExists, saveNote, ensureNotesDir, addRelation, buildVaultContextBlock, getAllSlugs, } from '../services/vault.service.js';
...
export function registerNoteCommand(program) {
    program
        ...
        try {
            ...
            let noteData;
            try {
                noteData = await queryClaudeCLIForNote(title, vaultContextBlock, config, attachments.length > 0 ? attachments : undefined);
            ...
            const existingRelated = noteData.relatedTerms.filter(rt => allSlugs.some(s => s.toLowerCase() === rt.toLowerCase()));
